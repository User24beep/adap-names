...
describe("Asserting not null or undefined", () => {
  it("test asserIsNotNullOrUndefined", async () => {
    ...
    expect(() =>
      IllegalArgumentException.assertIsNotNullOrUndefined(null)
    ).toThrow(new IllegalArgumentException(m));

    MethodFailedException.assertIsNotNullOrUndefined("hurray!");
    expect(() =>
      MethodFailedException.assertIsNotNullOrUndefined(null)
    ).toThrow(new MethodFailedException(m));

    InvalidStateException.assertIsNotNullOrUndefined("hurray!");
    expect(() =>
      InvalidStateException.assertIsNotNullOrUndefined(null)
    ).toThrow(new InvalidStateException(m));
  });
});
